...
def handle_object_appeared(evt, **kw):
    ...
    if isinstance(evt.obj, CustomObject):
        global event_appeared
        event_appeared = str(evt.obj.object_type)
        print("Cozmo started seeing a %s" % event_appeared)
...
def handle_object_disappeared(evt, **kw):
    ...
    if isinstance(evt.obj, CustomObject):
        global event_disappeared
        event_disappeared = str(evt.obj.object_type)          
...
def custom_objects(robot: cozmo.robot.Robot):
    ...
    global event_appeared
    while True:
        if event_appeared == "CustomObjectTypes.CustomType02":#hasTurn == False:
            #if event_appeared != event_disappeared:
            robot.turn_in_place(degrees(90)).wait_for_completed()
            event_appeared = " "
            # hasTurn = True
        elif event_appeared == "CustomObjectTypes.CustomType03":#hasTurn == False:
            #if event_appeared != event_disappeared:
            robot.turn_in_place(degrees(-90)).wait_for_completed()
            event_appeared = " "
            # hasTurn = True
        ...
        print(event_appeared, event_disappeared)
        time.sleep(1)
